...
public interface UpcCatalog {
  ...
  /** No catalog configured ({@code inventory.catalog=off}). */
  UpcCatalog OFF = gtin13 -> CompletableFuture.completedStage(Optional.empty());

  /** What the catalog knows about the code; empty on miss, outage, or limit. */
  CompletionStage<Optional<CatalogEntry>> lookup(String gtin13);
}
